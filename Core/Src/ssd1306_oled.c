...
void ssd1306_drawChar(const char *ch)
{
    ...
    for(uint32_t bitpos = 0; bitpos < len; bitpos++)
    {
        ...
        {
            i2c_start();
            i2c_request(SSD1306_SLAVE_ADDR_W);
            i2c_write(DATA_CTRL_BYTE);
            i2c_write( font[ch[ bitpos ] - 0x20] [bitmap] );
        }
    }
    i2c_stop();
}
...
void ssd1306_drawPixel(uint8_t x_pos, uint8_t y_pos)
{
    ...
    i2c_request(SSD1306_SLAVE_ADDR_W);
    i2c_write(DATA_CTRL_BYTE);
    i2c_write( *(p_ram + byte_pos) |= (0x01 << (y_pos % 8) )  );
    i2c_stop();
}
...
void ssd1306_displayClearPixel(uint8_t x_pos, uint8_t y_pos)
{
    ...
    i2c_request(SSD1306_SLAVE_ADDR_W);
    i2c_write(DATA_CTRL_BYTE);
    i2c_write( *(p_ram + byte_pos) = (0x00 << (y_pos % 8) )  );
    i2c_stop();
}
                           ...
                           SSD1306_PageNum_t page_start, SSD1306_PageNum_t page_end)
{
    i2c_start();
    i2c_request(SSD1306_SLAVE_ADDR_W);
    i2c_write(CMD_CTRL_BYTE);
    i2c_write(0x26 | dir);
    i2c_write(0x00);
    i2c_write(page_start);
    i2c_write(freq);
    i2c_write(page_end);
    i2c_write(0x00);
    i2c_write(0xFF);
...
void ssd1306_ramUpdateByte(uint16_t byte_pos, uint8_t byte_val)
{
    ...
    i2c_request(SSD1306_SLAVE_ADDR_W);
    i2c_write(DATA_CTRL_BYTE);
    i2c_write( *(p_ram + byte_pos) |= byte_val );
    i2c_stop();
}
...
/**
 * @brief    Initializes the I2C peripheral and the display
 * @param    none
 * @retval   none
 */
void ssd1306_init(void)
{
    ...
    /* Set Multiplex Ratio */
    i2c_write(0xA8);
    i2c_write(SSD1306_HEIGHT - 1); // 64 COM lines


    /* y axis */
    /* Set COM Output scan direction */
    // i2c_write(0xC0);    // COM0 - COM63
    i2c_write(0xC8); // COM63 - COM0
